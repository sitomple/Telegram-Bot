	...
	for id, gr in cells:
		if id.value == str(message.chat.id):
			if gr.value == 'ИСПк-101':
				...
				inlineMarkup = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True)
				item6 = telebot.types.InlineKeyboardButton("Понедельник")
				item7 = telebot.types.InlineKeyboardButton("Вторник")
				item8 = telebot.types.InlineKeyboardButton("Среда")
				item9 = telebot.types.InlineKeyboardButton("Четверг")
				item10 = telebot.types.InlineKeyboardButton("Пятница")
				item11 = telebot.types.InlineKeyboardButton("Суббота")
				inlineMarkup.add(item6, item7, item8)
				inlineMarkup.add(item9, item10, item11)
				bot.send_message(message.chat.id, 'Выберите день, на который хотите получить расписание:', reply_markup=inlineMarkup)
				teacherFile.answer(bot, message)
...
def teacher(bot, group, message):
	bot.send_message(message.chat.id, f'Активнен преподаватель: {message.text.strip()}')
	tumbler = 0
	i = 0
	wb = openpyxl.load_workbook('base.xlsx')
	sheet = wb['List']
...
def handle_text(message):
	...
	if (message.text.strip() == 'ИСПк-101-51-00') or (message.text.strip() == 'ИСПк-101'):
	...
	elif ((message.text.strip() == 'Студент') or (message.text.strip() == 'студент')):
		markup = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True)
		item1 = telebot.types.KeyboardButton("Help")
		item2 = telebot.types.KeyboardButton("Сегодня")
		item3 = telebot.types.KeyboardButton("Завтра")
		...
		markup.add(item1, item2, item3)
		markup.add(item4, item5)
		bot.send_message(message.chat.id, 'Выберите день, на который хотите получить расписание:', reply_markup=markup)

	elif ((message.text.strip() != 'Сегодня') and (message.text.strip() != 'Завтра') and (message.text.strip() != 'Послезавтра') and (message.text.strip() != 'Неделя') and (message.text.strip() != 'Понедельник') and (message.text.strip() != 'Вторник') and (message.text.strip() != 'Среда') and (message.text.strip() != 'Четверг') and (message.text.strip() != 'Пятница') and (message.text.strip() != 'Суббота') and (message.text.strip() != 'студент') and (message.text.strip() != 'Студент')) :
		...
		inlineMarkup.add(item6, item7, item8)
		inlineMarkup.add(item9, item10, item11)
		bot.send_message(message.chat.id, 'Выберите день, на который хотите получить расписание:',
						 reply_markup=inlineMarkup)
		teacher(bot, group, message)
	...
	elif message.text.strip() == 'Понедельник':
		day=0
		teacherFile.week_study(message, day, bot)
